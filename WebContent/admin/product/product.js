...
{
	s = t.value;
	//한글이나 영문이 들어가면 공백처리 해 줌
	s = s.replace(/\D/g, '');
	l = s.length - 3;
	
	//while문으로 반복되면서 생성하는것을 명심하자
	while (l > 0) {
...
function NumFormat2(s) // 원 단위, 찍어주기
{
	//한글이나 영문이 들어가면 공백처리 해 줌
	var s = s;
...
function go_ab() // 판매가-원가=순매출을 replace해서 계산해 준다.
{
	...
	ab = NumFormat2(ab.toString());//문자열만 length가 가능함
	if(ab == "NaN" || ab == null || ab == "" ){
		ab ="";
	}
...
function go_save() 
{
	...
	if (theForm.kind.value == '') {
		...
		theForm.encoding = "multipart/form-data";//파일을 업로드 할 수 있게 인코딩 방식을 정해줌.
...
function go_mod_save(tpage, pseq) {
	...
	if (theForm.kind.value == '') {
		...
		if (com == true) {
			...
			theForm.action = "NonageServlet?command=admin_product_update&tpage="+tpage+"";
			theForm.submit();
		}else{alert("수정을 취소 하셨습니다")}
	}
}
